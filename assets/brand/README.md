# Activos de marca Tazmany

`tazmany-logo.png` es una copia sin modificar del logotipo oficial vigente proporcionado el 27 de agosto de 2026. Su hash SHA-256 es:

```text
4998f9e9cfc4611fe821c13541fdd1943db1876636049203c0e2c1f4437cd1f3
```

La vista previa local lo carga desde esta carpeta. Apps Script no sirve automáticamente archivos binarios del repositorio; en el ambiente desplegado usa la URL pública controlada mediante `TAZMANY_LOGO_URL`. La URL incorpora la versión vigente como parámetro para evitar que el navegador conserve un logo anterior en caché.

La interfaz mantiene un wordmark de respaldo para que nunca aparezca una imagen rota. No se ha recortado, recoloreado, deformado ni redibujado la mascota.

## Relación entre marca e interfaz

El tema funcional aprobado para la interfaz es “Ahorro amarillo + ámbar”:

- Amarillo `#F2B705`: ahorro y acciones principales.
- Ámbar `#D77800`: promociones y energía comercial.
- Navy `#182635`: confianza, navegación y estructura.
- Crema `#FFF7D6`: fondos cálidos y cercanos.
- Grafito `#2A2D31`: descuentos y texto de alta importancia.

Estos colores no sustituyen los colores internos del logotipo. El archivo oficial usa navy para “Tazma” y ámbar para “ny”; el fallback textual replica esa relación con `#0A264E` y `#F2A000`.
